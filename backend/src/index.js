...
app.get("/quartos/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const { rows } = await pool.query(
      "SELECT * FROM quartos WHERE id_quarto = $1",
      [id]
    );
    if (rows.length === 0) {
...
app.post("/reservas", async (req, res) => {
  ...
  try {
    ...
    const { id_cliente, data_checkin, data_checkout, status, quartos } =
      req.body;
...
app.put("/reservas/:id", async (req, res) => {
  ...
  try {
    ...
    await client.query("DELETE FROM reservas_quartos WHERE id_reserva = $1", [
      id,
    ]);
